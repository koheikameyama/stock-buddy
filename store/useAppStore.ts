...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  fetchStockPrices: async (tickers: string[]) => {
    ...
    // キャッシュ有効なものは再利用（ただし currentPrice が null の場合は再取得）
    for (const ticker of tickers) {
      const cached = currentPrices.get(ticker) ?? null;
      if (
        isCacheValid(cached, CACHE_TTL.STOCK_PRICES) &&
        cached.data.currentPrice !== null
      ) {
    ...
    if (tickersToFetch.length > 0) {
      ...
      const responses = await Promise.allSettled(
        batches.map((batch) =>
          fetch(`/api/stocks/prices?tickers=${batch.join(",")}`).then((res) =>
  ...
  invalidatePortfolioSummary: () =>
    set({ portfolioSummary: null, nikkei: null }),
  invalidateAll: () =>
    set({
      userStocks: null,
      trackedStocks: null,
