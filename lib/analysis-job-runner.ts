...
- 利確目標（suggestedSellPrice）:
  - recommendationが"sell"の場合: 必ず現在価格を設定する（「今すぐ売れ」という意味なので）
  - recommendationが"hold"で含み益がある場合: 現在の利益を確保しつつ、さらなる上昇余地を考慮した目標価格
  - recommendationが"hold"で含み損がある場合: 平均取得単価への回復を目標とするか、市場分析に基づく現実的な水準
- 損切りライン（suggestedStopLossPrice）:
  - recommendationが"sell"の場合: nullを設定する（売却を勧めているので損切りラインは不要）
  - recommendationが"hold"で含み益がある場合: 利益が消えないライン（例：平均取得単価の少し上）
  - recommendationが"hold"で含み損がある場合: これ以上の損失拡大を防ぐライン（例：現在価格から-5%〜-10%）
