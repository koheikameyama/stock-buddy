...
■ 慎重派（CONSERVATIVE）: 早めの利確・損切りを重視 + リスク限定型の判断
- ユーザー心理: 「損をしたくない」人。ただし「稼ぎたくない」わけではない。理想は「最悪のシナリオ（大損）が封じ込められた上での堅実なプラス」
- adviceでは「最悪のケースでどれくらい損をするか（最大損失率・概算損失額）」を明示した上で、保有継続・売却・買い増しの判断理由を説明する
  - 例: 「撤退ラインを〇〇円に設定すれば最大損失を約△%に限定でき、売却目標□□円で約◇%の利益が狙えます」
...
- ★ ただし「リスク・リワード比（撤退ラインまでの下落幅に対して売却目標までの上昇幅）」が1:3以上見込める場合:
  ...
  - 赤字銘柄でもセクター全体に買いが入っていれば「地合いを味方につけた短期戦」として hold 継続を提案可能（cautionで業績リスクに必ず言及）
  - 例: 「支持線〇〇円が維持されればリスク△%で□%の反発が見込めます。損切りラインを厳格に守りつつ保有継続を検討しましょう」
...
- アドバイスのトーン: 「リスクは限定的です。撤退ラインを厳守しつつ慎重に判断しましょう。」

■ バランス型（BALANCED）: 中期トレンドで判断 + トレンド転換を捉える
...
■ 積極派（AGGRESSIVE）: 利益最大化を重視 + モメンタム重視
