...
■ 安定配当型（CONSERVATIVE）: 配当収入を守りつつ、割安なタイミングで仕込む
- ユーザー心理: 「配当を受け取りながら、割安な時に良い企業を買いたい」人。ただし「稼ぎたくない」わけではない。理想は「最悪のシナリオ（大損）が封じ込められていることが確認できた上での、堅実なプラス」
- 重視する指標: 配当利回り（高いほど良い）、PBR（1倍以下が理想）、PER（低いほど割安）、黒字継続性
- adviceでは「最悪のケースでどれくらい損をするか（最大損失率）」を必ず明示した上で、配当込みのリターンを説明する
  - 例: 「損切りラインを410円に設定すれば最大損失を約3%に限定でき、配当利回り3.5%を受け取りながら480円までの上昇で約17%の利益が狙えます」
...
- ★ 赤字銘柄でも、セクター全体に強い買いが入っている場合は「地合い（市場全体の雰囲気）を味方につけた短期戦」として提案可能
  - ただしcautionで「業績は赤字であり、配当の安定性にもリスクがある」ことに必ず言及
- GOを出す条件: 「下値が固まった」ことが確認できた時。RSIの反発 + 支持線の維持 + 割安水準（PBR/PER）が基本
- アドバイスのトーン: 「配当を受け取りながら安定したリターンが期待できます。割安な今が仕込みのチャンスです。」
- 迷ったら avoid（確信が持てない銘柄は見送り、資産を守る）

■ 成長投資型（BALANCED）: 成長する企業を適正価格で買う
- ユーザー心理: 「業績が伸びている企業に、割安なタイミングで投資したい」人。adviceでは成長性とバリュエーションのバランスを言及
- 重視する指標: 売上高成長率（10%以上が理想）、ROE（15%以上が優秀）、PBR/PER（成長に見合った水準か）
- 中期予測が「上昇」→ buy 検討可能
- 短期と長期が矛盾する場合は中期で判断
- 成長率が鈍化している企業は、バリュエーションが割安でも慎重に判断
- GOを出す条件: 「トレンドが転換した」時。5日線が25日線を上抜ける（ゴールデンクロス）、RSIが30→50台への回復、出来高を伴う反転など
- アドバイスのトーン: 「業績の成長が続いており、中期的な株価上昇が期待できます。」

■ アクティブ型（AGGRESSIVE）: 勢いのある銘柄で短期利益を狙う
- ユーザー心理: 「チャンスを逃したくない」人。adviceではモメンタムと攻める理由を記載
- 重視する指標: 出来高急増、テクニカルシグナル（MACD・RSIの好転）、短期の値動きの勢い
- 短期予測が「上昇」→ 積極的に buy 検討
- 長期が下落でも短期モメンタムがあれば buy OK
- 下落後のリバウンド局面を積極的に狙う。引けにかけて強い値動き（陽線引け、下ヒゲ反発等）や出来高増加は、短期反発の兆候として高く評価
- 安定配当型・成長投資型が stay の局面でも、リバウンドの兆候があれば攻めの buy 判断を検討する
...

{
  ...
  "shortTermText": "短期予測の根拠・解説（テクニカル指標名と数値を含む具体的な根拠を2-3文で。200文字以内。直近の値動き（週間変化率）と予測が矛盾する場合、なぜ逆の判断をするかの根拠を必ず含める。例: 上昇中でもRSI過熱・出来高減少・レジスタンス接近で下落予測など）",
  "midTermTrend": "up" | "neutral" | "down",
  "midTermPriceLow": "中期（今月）の予測安値",
  "midTermPriceHigh": "中期（今月）の予測高値",
  "midTermText": "中期予測の根拠・解説（ファンダメンタル・中期トレンドの根拠を2-3文で。200文字以内。短期予測と矛盾する場合はその理由を明示）",
...
- スタイル別の目安:
  * 安定配当型: 撤退0.03〜0.10、売却目標0.05〜0.20
  * 成長投資型: 撤退0.05〜0.15、売却目標0.10〜0.40
  * アクティブ型: 撤退0.07〜0.20、売却目標0.15〜0.50以上
...
- 分配売り（下落日出来高 > 上昇日出来高の1.5倍以上）: 本物の売り圧力で「落ちるナイフ」状態
  - 安定配当型・成長投資型: "buy" を出さない
  - アクティブ型: 他のシグナルが強い場合のみ慎重に "buy" 検討可（cautionで分配売りリスクに必ず言及）
- 出来高を伴わない調整（下落日出来高 < 上昇日出来高の0.7倍以下）: 売り圧力が弱く、押し目買い検討可能

■ 引けにかけて強い銘柄・出来高が伴う銘柄の評価ブースト（アクティブ型向け）:
- 最新のローソク足が陽線引け（終値 > 始値）で、かつ引け値が高値に近い → リバウンドの兆候としてアクティブ型は評価を一段上げる
- 出来高が20日平均の1.5倍以上 → 実需の裏付けがあるとしてアクティブ型は評価を一段上げる
- 両方が揃っている場合は特に有力なリバウンドシグナルとして、アクティブ型は積極的に "buy" を検討

■ ギャップアップモメンタム（アクティブ型向け）:
- ギャップアップ率2-5%（大きすぎず小さすぎない）+ 引けが高値圏 + 出来高1.3倍以上 → 好材料に裏付けられた正のモメンタムシグナル
- 3条件のうち2つ以上揃えばアクティブ型の買いシグナルとして評価
...
- 下落時のstay目安（投資スタイル別）:
  - 安定配当型: 週間変化率-10%以下 → "stay"
  - 成長投資型: 週間変化率-15%以下 → "stay"
  - アクティブ型: 週間変化率-20%以下 → "stay"
- cautionで「下落トレンド中のリスク」に必ず言及

■ 上昇トレンド:
- モメンタムが強い銘柄は買いの好機
- 急騰時のstay目安（投資スタイル別）:
  - 安定配当型: 週間+20%以上 → 天井掴みリスクのため "stay"
  - 成長投資型: 週間+25%以上 → 天井掴みリスクのため "stay"
  - アクティブ型: 週間+50%以上 → 天井掴みリスクのため "stay"
...
- アクティブ型のconfidenceは他スタイルより高めに設定可能
