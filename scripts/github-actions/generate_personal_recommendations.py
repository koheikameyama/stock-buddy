...
# 除外ではなくペナルティにすることで、高リスク志向ユーザーには表示される可能性を残す
RISK_PENALTY = {
    "high": -10,    # 高リスク志向: -10点（軽めのペナルティ）
    "medium": -20,  # 中リスク志向: -20点
    "low": -30,     # 低リスク志向: -30点（厳しめのペナルティ）
}
...
def calculate_stock_scores(
    ...
    penalty = RISK_PENALTY.get(risk or "medium", -20)
    excluded_count = 0
    penalty_count = 0
    ...
    for stock in stocks:
        ...
        # 赤字 AND 高ボラティリティの場合はペナルティ（順位を下げるが表示は可能）
        is_high_risk_stock = (
    ...
    if excluded_count > 0:
        print(f"  Excluded {excluded_count} stocks (weekChange>50%)")
    if penalty_count > 0:
