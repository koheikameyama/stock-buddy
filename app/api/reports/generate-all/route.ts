...
/**
 * 全ユーザーの日次レポートを一括生成
 * GitHub Actionsのcronジョブから呼び出される
 */
    ...
    for (const user of users) {
      try {
        ...
        if (existingReport) {
          ...
          console.log(`[User: ${user.email}] 既存レポートを削除`)
        }
          ...
          portfolio.stocks.map(async (ps) => {
            try {
              ...
              if (priceData.length === 0) {
                console.warn(`[User: ${user.email}] No price data for ${ps.stock.tickerCode}`)
                return null
              }
              ...
              console.error(`[User: ${user.email}] Error processing ${ps.stock.tickerCode}:`, error)
              return null
            }
          })
        )
        ...
        if (validStocks.length === 0) {
          console.warn(`[User: ${user.email}] 株価データの取得に失敗`)
          errorCount++
          errors.push(`${user.email}: 株価データの取得に失敗`)
          continue
        }
...
${marketNews ? "市場動向も考慮して、" : ""}JSON形式で返してください。`,
        ...
        if (!openaiResponse.ok) {
          ...
          console.error(`[User: ${user.email}] OpenAI API error:`, errorText)
          errorCount++
          errors.push(`${user.email}: AI分析に失敗`)
          continue
        }
        ...
        console.log(`[User: ${user.email}] レポート生成成功`)
        successCount++
      } catch (error) {
        console.error(`[User: ${user.email}] Error generating report:`, error)
        errorCount++
