...
【一部売却（partial_sell）の条件】
- 日本株は100株単位（単元株）での取引が基本
- 保有数が200株未満の場合、一部売却は物理的にできないため「partial_sell」は推奨しない
- 保有数が200株以上の場合のみ「partial_sell」を選択肢として検討する
- 一部売却を推奨する場合: 利益確定しつつ上昇余地も狙いたい、またはリスクを減らしつつ保有継続したい状況
