...
export default function StockCard({
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all p-6 relative">
      ...
      {/* Portfolio Analysis Toggle (portfolio only) */}
      {isHolding && (
        <button
          onClick={() => setShowPortfolioAnalysis(!showPortfolioAnalysis)}
          className="mt-4 w-full px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg font-semibold hover:bg-indigo-100 transition-colors flex items-center justify-center gap-2"
        >
          <svg
            className="w-5 h-5"
            fill="none"
          ...
          {showPortfolioAnalysis ? "売買判断を非表示" : "📊 売買判断を見る"}
        </button>
      )}
