...
| 用途 | ツール | 理由 |
|------|--------|------|
| ワンクリック系（数を数える） | Google スプレッドシート | 集計・グラフ化が簡単 |
| テキスト要望（内容を読む） | Notion | 分類・タグ付けが簡単 |

#### 収集方法

**1. ワンクリックボタン → スプレッドシート**

機能制限に達した時に表示:
```
もっと銘柄を追加したいですか？
[はい、増やしたい] ← クリックでスプシに記録
```
...
実装: Google Apps Script で Webhook 作成 → fetch で POST
