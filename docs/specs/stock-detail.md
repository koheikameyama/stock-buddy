...
| ATR(14) | 14日間の平均真の値幅（Average True Range） | ボラティリティ指標。値が大きいほど値動きが激しい |
...
| 指標 | 計算方法 | 判定基準 |
|------|--------|----------|
| **ギャップアップ率** | (当日始値 - 前日終値) / 前日終値 × 100 | > +5%: 過熱注意 / > +10%: 飛びつき買い防止で強制stay |
| **出来高急増率** | 当日出来高 / 20日平均出来高 | > 1.5倍: 注目度上昇 / > 2.0倍: 材料出現の可能性 / > 5.0倍(+ギャップ5%以上): 仕手株リスクで強制stay |
| **売買代金** | 出来高 × 終値 | 流動性の目安（億円/万円単位で表示） |

#### ニュースタブ

- 直近14日間の関連ニュース（最大10件）
- 銘柄固有ニュースと関連セクターニュースを混合表示（日付順）
- センチメント表示（positive / negative / neutral）
- マッチタイプ表示（セクターニュースには「同セクター」バッジ）
- 取得ロジック:
  1. yfinance で直接取得した銘柄紐付きニュース（`tickerCode` 直接マッチ）
  2. RSS ニュース本文に銘柄コードが含まれるもの（テキストマッチ）
  3. 同一セクターのニュース（常時取得、最低3件）
...
| 時価総額 | 発行株数×株価 |
| EPS | 1株当たり利益 |
| 負債比率（D/E Ratio） | 総負債÷自己資本。低いほど財務が健全 |
| 流動比率 | 流動資産÷流動負債。短期的な支払い能力を示す |
| 配当成長率 | 前年比の配当増減率（%） |
| 配当性向 | 利益のうち配当に回す割合（%）。高すぎると減配リスク |
| 売上高 / 純利益 | 直近通期 |
| 売上高成長率 / 純利益成長率 | 前年比 |
| 次回決算日 | カウントダウンバッジ付き |
...
銘柄のPER/PBR/ROEをセクター平均と比較し、割安/割高/平均的を色分け表示。
- PER/PBR: セクター平均より10%以上低い→割安（緑）、10%以上高い→割高（赤）
- ROE: セクター平均より10%以上高い→効率的（緑）、10%以上低い→平均以下（赤）
...
仮保有（100株）での分析シミュレーション。投資スタイル別分析（安定配当型/成長投資型/アクティブ型）も生成し、`styleAnalyses` として返却。
...
関連ニュース取得（14日分、最大10件）。銘柄固有ニュースと関連セクターニュースを混合表示。
