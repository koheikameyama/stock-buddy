...
| `sell_target` | 売却目標到達 | ユーザー設定の売却目標価格に到達（スタイル別AI判断をメッセージに付加） |
| `stop_loss` | 撤退ライン到達 | ユーザー設定の撤退ラインに到達（スタイル別AI判断をメッセージに付加） |
| `profit_milestone` | 利確マイルストーン通知 | ポートフォリオ銘柄の含み益が+10%, +20%, +30%に到達した際に通知。投資スタイル別のアドバイス付き |
| `switch_proposal` | 乗り換え提案 | スマートスイッチが含み損銘柄の乗り換え先を検出した際に通知。switchBenefit が閾値（`SMART_SWITCH.NOTIFICATION_THRESHOLD` = 40）以上の場合のみ |
| `market_shield_activated` | マーケットシールド発動 | 市場急変を検知しマーケットシールドが発動した際の緊急通知。トリガー種別（日経225急落/VIX急騰/WTI急変動/為替急変動）と値を含む |
| `market_shield_deactivated` | マーケットシールド解除 | マーケットシールドが解除された際の通知。morningセッションでの自動解除時に送信 |
| `trend_convergence` | トレンド収束 | 短期/長期トレンドの乖離が検出された際の通知。収束予測日数と注目価格水準を含む |
...
通知生成時にユーザーの投資スタイル（安定配当型/成長投資型/アクティブ型）の分析結果を参照する。
