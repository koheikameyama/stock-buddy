...
2. ユーザーごとに並列処理（最大3並列）:
   ...
   ※ 保有銘柄・ウォッチリスト銘柄は除外しない（追加購入・購入後押しの候補として残す）
...
- 保有銘柄が候補に含まれる場合、AIプロンプトに保有情報を伝え「追加購入」の文脈で理由を生成
  - AIは reason の冒頭に「【追加購入チャンス】」を付与
  - フロントエンドでは「買い増しチャンス」バッジを表示（青色）
- ウォッチリスト銘柄が候補に含まれる場合、AIプロンプトにウォッチリスト情報を伝え「買い時」の文脈で理由を生成
  - AIは reason の冒頭に「【注目銘柄が買い時】」を付与
