...
**新機能（スキーマ実装済み）:**
1. **買い時アラート機能**
   - `targetPrice`: 目標購入価格（この価格以下で通知）
   - `priceAlert`: 価格アラート有効/無効
   - `lastAlertSent`: 最後にアラート送信した日時
   - `buyTimingScore`: 買い時スコア（0-100）投資スタイル考慮
   - `lastAnalyzedAt`: 最後に分析した日時

2. **仮想購入シミュレーション機能**
   - `virtualBuyPrice`: 仮想購入価格
   - `virtualBuyDate`: 仮想購入日
   - `virtualQuantity`: 仮想購入数量
   - 「もし買っていたら？」のシミュレーション

**ポートフォリオとの差別化:**
- ウォッチリスト: 「観察 → 買い時判断 → 練習」
- ポートフォリオ: 「保有 → 売買判断 → 実績管理」
